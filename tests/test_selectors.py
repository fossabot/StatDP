...
def test_select_event():
    ...
    _, _, _, event = select_event(noisy_max_v1a, [(d1, d2, {'epsilon': 0.5})], 0.5, 100000, cores=0)
    assert 0 in event
    _, _, _, event = select_event(noisy_max_v1b, [(d1, d2, {'epsilon': 0.5})], 0.5, 100000, cores=0)
    assert 0 in event
